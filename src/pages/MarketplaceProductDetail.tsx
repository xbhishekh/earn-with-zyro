...
const MarketplaceProductDetail = () => {
  ...
  const handlePurchase = async () => {
    ...
    if (finalPrice > 0 && userBalance < finalPrice) {
      toast.error("Insufficient balance. Please add funds to your CliporaX Balance.", {
        action: {
          label: "Add Funds",
  ...
  const productDescription = product.short_description || product.description 
    ...
    : `${product.title} by ${seller?.display_name || 'CliporaX seller'} - ${product.price === 0 ? 'Free' : `$${product.price}`}. Join ${product.members_count.toLocaleString()} members on CliporaX Marketplace.`;
  
  const seoTitle = product.price === 0 
    ? `${product.title} - Free on CliporaX Marketplace`
    : `${product.title} - $${product.price} | CliporaX Marketplace`;
